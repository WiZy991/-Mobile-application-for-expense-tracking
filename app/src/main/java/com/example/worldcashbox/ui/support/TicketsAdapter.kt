...
import com.example.worldcashbox.databinding.ItemTicketBinding
import java.text.SimpleDateFormat
import java.util.Locale

class TicketsAdapter(
    private val onTicketClick: (Ticket) -> Unit
) : ListAdapter<Ticket, TicketsAdapter.TicketViewHolder>(
    TicketDiffCallback()
) {
    ...
    class TicketViewHolder(
        ...
        fun bind(ticket: Ticket) {
            ...
            binding.ticketStatusTextView.text = when (ticket.status) {
                ...
                "done" -> "Выполнено"
                "closed" -> "Закрыто"
                "open" -> "Открыт" // Старый статус для обратной совместимости
                "resolved" -> "Решен" // Старый статус для обратной совместимости
                else -> ticket.status
            }
            ...
            val statusColor = when (ticket.status) {
                ...
                "open" -> binding.root.context.getColor(com.example.worldcashbox.R.color.warning) // Старый статус
                "resolved" -> binding.root.context.getColor(com.example.worldcashbox.R.color.success) // Старый статус
